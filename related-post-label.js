function labelthumbs(t) {
	for (var e = 0; e < numposts; e++) {
		var m, n = t.feed.entry[e],
		i = n.title.$t;
		if (e == t.feed.entry.length) break;
		for (var r = 0; r < n.link.length; r++) {
			if ("replies" == n.link[r].rel && "text/html" == n.link[r].type) var l = n.link[r].title,
			o = n.link[r].href;
		...
		w = "ba";
		try {
			h = n.media$thumbnail.url,
			h = h.replace("/s72-c/", "/w" + thumb_width + "-h" + thumb_height + "-c/")
		} catch(_) {
			s = n.content.$t,
			a = s.indexOf("<img"),
			b = s.indexOf('src="', a),
		...
		$ = y.substring(5, 7),
		C = y.substring(8, 10);
		document.write('<ul class="huba-gambar">'),
		document.write("<li>"),
		1 == showpostthumbnails && document.write('<a title="' + i + '" href="' + m + '"><div class="huba-gambar"><span class="rollover"></span><img width="' + thumb_width + '" height="' + thumb_height + '" title="' + i + '" alt="' + i + '" src="' + h + '"/></div></a>'),
		document.write('<span class="huba-title"><a title="' + i + '" href="' + m + '">' + i + "</a></span>");
		var O = "og",
		M = "sp",
		N = "ot.",
		R = "";
		...
		var I = /<\S[^>]*>/g;
		if (z = z.replace(I, ""), 1 == showpostsummary) if (z.length < numchars) document.write(""),
		document.write(z),
		document.write("");
		else {
			document.write(""),
			z = z.substring(0, numchars);
			var S = z.lastIndexOf(" ");
			z = z.substring(0, S),
			document.write(z + "..."),
			document.write("")
	...
	document.write('<ul class="huba-gambar2">');
	for (var e = 1; e < numposts2; e++) {
		var m, n = t.feed.entry[e],
		i = n.title.$t;
		if (e == t.feed.entry.length) break;
		for (var r = 1; r < n.link.length; r++) {
			if ("replies" == n.link[r].rel && "text/html" == n.link[r].type) var l = n.link[r].title,
			o = n.link[r].href;
			if ("alternate" == n.link[r].rel) {
				m = n.link[r].href;
		...
		try {
			...
			c = s.indexOf('"', b + 5),
			d = s.substr(b + 5, c - b - 5),
			j = -1 != a && -1 != b && -1 != c && "" != d ? d: no_thumb2
		}
		var y = n.published.$t,
		x = y.substring(0, 4),
		$ = y.substring(5, 7),
		C = y.substring(8, 10);
		1 == showpostthumbnails2 && document.write('<a title="' + i + '" href="' + m + '"><div class="huba-gambar2"><img width="' + thumb_width2 + '" height="' + thumb_height2 + '" title="' + i + '" alt="' + i + '" src="' + j + '"/></div></a>'),
		document.write("<li>"),
		document.write('<span class="huba-title huba-title2"><a title="' + i + '" href="' + m + '">' + i + "</a></span>");
		var R = "";
